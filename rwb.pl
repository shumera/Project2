...
if ($action eq "near") {
  ...
  # Summary statistics for aggregation portion
  my @col;
  my $commToCommTotal;
  my $commToCandTotal;
  my $indToRepCommTotal;
  my $indToDemCommTotal;
  my $commToRepCandTotal;
  my $commToDemCandTotal;
  my $commToRepCommTotal;
  my $commToDemCommTotal;
  my $demTotal;
  my $repTotal;
  my $total;
  my $demPercent;
  ...
  if ($what{candidates}) {
    my ($str,$error) = Candidates($latne,$longne,$latsw,$longsw,$cycle,$format,@sqledl);
    if (!$error) {
      if ($format eq "table") { 
  ...
  if ($what{individuals}) {
    my ($str,$error) = Individuals($latne,$longne,$latsw,$longsw,$cycle,$format,@sqledl);
    if (!$error) {
      if ($format eq "table") { 
  ...
  if ($what{opinions}) {
    my ($str,$error) = Opinions($latne,$longne,$latsw,$longsw,$cycle,$format,@sqledl);
    if (!$error) {
      if ($format eq "table") { 
